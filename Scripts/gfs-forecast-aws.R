...
#'  
#' gfs_forecast_aws(date_str, date_end, resolution, interval, issued_time)
#' }
#' @export
gfs_forecast_aws <- function(date_str, date_end, res, interval, issued_time) {
  # export library
...
# Insert environment from bash terminal
date_str    <- Sys.getenv("START_TIME")   #"2021-08-23 00:00:00"
date_end    <- Sys.getenv("END_TIME")     #"2021-08-26 00:00:00"
resolution  <- Sys.getenv("RES")          #"1p00"
interval    <- Sys.getenv("INTERVAL")     # 6
issued_time <- Sys.getenv("ISSUED_TIME")  # 0

# Check if Sys.getenv() empty
if (date_str == "" | date_end == "" | resolution == "" | issued_time == "" | interval == "") {
  stop("Please type Environment Variable: START_TIME, END_TIME, RES, INTERVAL, & ISSUED_TIME !!!")
}
